...
def RS_ester_reactants():
    ...
    reactants = pd.DataFrame({"smiles_monomer": smiles, "replicate_structures": [1]}, columns=["smiles_monomer", "replicate_structures"], index=poly_name)
...
def RS_vinyl_reactants():
    ...
    reactants = pd.DataFrame({"smiles_monomer": smiles, "replicate_structures": [1]}, index=poly_name)
    reactants["monomers"] = reactants.smiles_monomer.apply(
        lambda s: pm.get_monomers(s, stereochemistry=True)
    )
