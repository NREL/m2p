...
from rdkit.Chem import AllChem, CanonSmiles, Descriptors
...
class PolyMaker:
    ...
    def get_monomers(self, s, stereochemistry=False):
        ...
        if type(s) == str:
            ...
            if not stereochemistry:
                s = [s_i.replace("/", "").replace("@", "") for s_i in s]
    ...
    def thermoplastic_stereo(
        ...
        def get_stereo_smiles_from_diads(diad_string, mol, chiral_centers):
            ...
            mol.GetAtoms()[chiral_centers[0]].SetChiralTag(
                chiral_type[current_chirality]
            )
            # if diad_string[0] == "1":
            current_chirality += 1
            ...
            for i, diad in enumerate(diad_string):
                if diad == "1":
                    mol.GetAtoms()[chiral_centers[i + 1]].SetChiralTag(
                        chiral_type[current_chirality % 2]
                    )
                else:
                    current_chirality += 1
                    mol.GetAtoms()[chiral_centers[i + 1]].SetChiralTag(
        ...
        def generate_stereo_ester(smi, n_structures, pm):
            ...
            smiles_list = [
                get_stereo_smiles_from_diads(diad_string, mol, chiral_centers)
                for diad_string in diads_list
            ]
        ...
        nostereo_df.loc[:, "smiles"] = nostereo_df.smiles.map(
            strip_smiles_stereochemistry
        )
        nostereo_df.loc[:, "monomers"] = nostereo_df.monomers.map(
            strip_monomers_stereochemistry
        )
        ...
        # Find duplicates based on polymer, pm, and mechanism
        duplicates_df = (
            poly_df.groupby(by=["pm", "polymer"])
            .size()
            .to_frame("n_rows")
            .reset_index()
        )
        for _, row in duplicates_df.iterrows():
            smiles = generate_stereo_ester(row.polymer, row.n_rows, row.pm)
            smiles_df = pd.DataFrame(data={"smiles": smiles})
            poly_df.loc[
                (poly_df.polymer == row.polymer) & (poly_df.pm == row.pm), "polymer"
            ] = smiles_df.smiles.to_numpy()
    ...
    def __poly_ester(self, reactants, DP=2, distribution=[], infinite_chain=False):
        ...
        try:
            ...
            while DP_count < DP:
                ...
                assert (
                    df_func.loc["polymer"][df_func.loc["polymer"] > -1].shape
                    == df_func.loc["polymer"].shape
                )
                ...
                if (infinite_chain) & (DP_count == DP - 1):
                    ...
                    if (df_func.loc["polymer", "aliphatic_ols"] > 0) & (
                        df_func.loc["polymer", "acids"]
                    ) > 0:
    ...
    def __poly_amide(self, reactants, DP=2, distribution=[], infinite_chain=False):
        ...
        try:
            ...
            while DP_count < DP:
                ...
                # select rxn rule and reactant
                if (df_func.loc["polymer", "prime_amines"] >= 1) & (
                    df_func.loc["polymer", "acids"] >= 1
                ):
                ...
                if (infinite_chain) & (DP_count == DP - 1):
                    # logic for closing ring
                    if (df_func.loc["polymer", "prime_amines"] > 0) & (
                        df_func.loc["polymer", "acids"]
                    ) > 0:
    ...
    def __poly_carbonate(self, reactants, DP=2, distribution=[], infinite_chain=False):
        ...
        try:
            ...
            while DP_count < DP:
                ...
                if (infinite_chain) & (DP_count == DP - 1):
                    # logic for closing ring
                    if (df_func.loc["polymer", "ols"] > 0) & (
                        df_func.loc["polymer", "carbonates"] > 0
                    ):
                        # case for when has can ring close
                        DP_count += 1
                        DP_actual += 1
        ...
        try:
            ...
            while DP_count < DP:
                ...
                if (infinite_chain) & (DP_count == DP - 1):
                    # logic for closing ring
                    if (df_func.loc["polymer", "prime_amines"] > 0) & (
                        df_func.loc["polymer", "acidanhydrides"]
                    ) > 0:
                        # case for when has can ring close
                        DP_count += 1
                        DP_actual += 1
    ...
    def __poly_upe(self, reactants, crosslinker, distribution, DP):
        ...
        while ring1 == "ERROR" or ring2 == "ERROR":
            ...
            if dfrings.polymer.str.contains(
                "ERROR"
                ...
                mol, p = dfrings.apply(
                    lambda row: self.__protect_substructure(
                        row.polymer, "C=C", n_unprotected=1
                    ),
                    axis=1,
                )[0]
                if p > 0:
