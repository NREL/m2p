...
class PolyMaker():
    ...
    def thermoplastic_stereo(self, reactants, DP=2, mechanism='', replicate_structures=1, distribution=[], pm=1, verbose=True):
        ...
        def generate_stereo_ester(smi, n_structures, pm):
            ...
            shortest_path = set(AllChem.rdmolops.GetShortestPath(mol, ol_atoms[0][0], acid_atoms[0][0],))
        ...
        duplicates_df = poly_df.groupby(by=["pm", "polymer"]).size().to_frame('n_rows').reset_index()
        for _, row in duplicates_df.iterrows():
            smiles = generate_stereo_ester(row.polymer, row.n_rows, row.pm)
            smiles_df = pd.DataFrame(data={"smiles": smiles})
            poly_df.loc[(poly_df.polymer == row.polymer) & (poly_df.pm == row.pm), "polymer"] = smiles_df.smiles.to_numpy()
